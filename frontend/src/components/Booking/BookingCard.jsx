...
import { 
  CalendarToday, Hotel, PaymentsOutlined, AccessTime
...
export default function BookingCard({ booking, onView }) {
  ...
  // Lấy đầy đủ các trường từ object booking
  const { 
    status, 
    paymentStatus, 
    totalPrice, 
  ...
  // ✅ LOGIC TÍNH TOÁN CHUẨN: Không chỉ dựa vào paymentStatus string
  ...
  // 1. Badge hiển thị trạng thái thanh toán (Dưới tên khách sạn)
  ...
  // 2. Status Tag hiển thị ở góc ảnh
  const getStatusTag = () => {
          ...
          <Box flex={1}>
            ...
            <Stack direction={isMobile ? "column" : "row"} justifyContent="space-between" alignItems="flex-end" spacing={2}>
              <Box>
                ...
                {/* HIỂN THỊ CHI TIẾT SỐ TIỀN CÒN LẠI NẾU MỚI CỌC */}
                {isOnlyDeposited && (
